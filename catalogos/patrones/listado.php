...
<?php 
    ...
    if($campos_url['idpatron']!=""){       $filtro  =	" AND idpatron='".$campos_url['idpatron']."'"; 				}
    ...
    if($orden!=""){       $filtro .= 	" ORDER BY nombre_patron ".$orden;	}
	if($nregistros!=""){  $filtro .= 	" LIMIT ".$nregistros;				}
	
?>   
    
<?php 
	$nrows      		=   $datosmysql->totalRows("SELECT * FROM cat_patron WHERE 1".$filtro);       	
	if($nrows<=0){		echo '<div class="well well-lg">No Existen registros</div>';		}	
    else{
      		$query =   $datosmysql->Query("SELECT * FROM cat_patron WHERE 1".$filtro); 
?>
...
<table class='table table-hover fixed'>
	...
	<thead>
		<tr><td>HERRAMIENTAS</td><td>ID</td><td>NOMBRE PATRON</td><td>CULTIVO</td><td>% AUMENTO</td></tr>
	</thead>
			...
			<tr class="active">
				<td>
				   <div class="btn-group" role="group" aria-label="...">				        
				  	    <a href="#" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="top" title="Eliminar registro"  onclick="Eliminar(<?php print $d["idpatron"];?>)"><span class="glyphicon glyphicon-trash" aria-hidden="true" ></span></a>
				  	    <a href="#" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="top" title="Editar registro"  onclick="Formulario(<?php print $d["idpatron"];?>)"><span class="glyphicon glyphicon-pencil" aria-hidden="true" ></span></a>
				   </div>  	        		    				   
			    </td>
				<td ><?php print $d["idpatron"];?></td>
